...
Version 1.6 adds safe cycle restart:
12. Reset the 28-day cycle to Day 1 without manually editing the state file
...
#### 8. `restart`
Resets the 28-day cycle to Day 1 from today's date.

Behavior:
- requires a confirmation step before applying (confirm / cancel inline buttons)
- clears all completed, skipped, and manual-rest-day history
- resets the start date to today
- keeps reminder settings unchanged (time, timezone, Shabbat skip)
- cannot be undone
- chatId is preserved

Use case: Moshe fell off the plan for an extended period and wants a clean start without manually editing the JSON state file.
...
- commands stay in English: `today`, `done`, `skip`, `status`, `swap`, `restart`
